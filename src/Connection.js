export default function Connection(code) {
    ...
    this.insertMessage = (message) => {
        const suffix = message.source === 'own' ? ' (me)' : '';
        chatLog.insertAdjacentHTML(
        "beforeend",
        `<li id="c${message.id}" class="${message.source}">
                <em>
                    <img 
                    src="https://unavatar.now.sh/github/${escapeHtml(message.user)}"
                    alt="Unavatar avatar"
                    >
                    ${escapeHtml(message.user)}${suffix}: 
                </em>
    ...
    this.insertJoined = (message) => {
        chatLog.insertAdjacentHTML(
            "beforeend",
            `<li class="welcome">${escapeHtml(message.player.user)} has joined the chat</li>`
        );
        this.updateScroll();
    };
    ...
    this.insertLeft = (message) => {
        chatLog.insertAdjacentHTML(
            "beforeend",
            `<li class="goodbye">${escapeHtml(message.name)} has left the chat</li>`
        );
        this.updateScroll();
    };
